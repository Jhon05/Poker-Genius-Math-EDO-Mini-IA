...
class LocalModelProvider{
 ...
 createWorker(){if(this.worker)return;const epoch=++this.epoch;this.worker=new Worker(G.config.localModel.worker,{type:'module',name:'GeniusMathInference'});
  ...
  this.worker.onerror=e=>{const error=Object.assign(Error(e.message||'El navegador bloqueó o perdió el Worker.'),{stage:'worker',code:'WORKER_CRASH'});this.fail(error)};
  this.worker.onmessageerror=()=>this.fail(Object.assign(Error('No se pudo decodificar un mensaje del Worker.'),{stage:'worker',code:'WORKER_MESSAGE_ERROR'}));
 }
 failTimeout(id,message){if(!this.pending.has(id))return;this.fail(Object.assign(Error(message),{code:'TIMEOUT',stage:this.stage}))}
 ...
 async initialize(profile){
  ...
  try{
   ...
   if(!cap.supported){const first=cap.checks.find(x=>x.required&&x.status!=='pass')||{code:'UNSUPPORTED',message:'El dispositivo no superó la comprobación segura.',remedy:'Continúa con la Guía compatible.'};throw Object.assign(Error(first.message),{diagnostic:{code:first.code,cause:first.message,impact:first.impact||'El proveedor local no puede arrancar en este entorno.',remedy:first.remedy,continueWithoutAI:true,stage:'capabilities'}})}
   profile=profile||cap.recommendation?.profile||G.config.localModel.profile||'light';const cfg=G.config.profiles[profile];if(!cfg)throw Object.assign(Error('Perfil de modelo no incluido en esta compilación.'),{code:'PROFILE_NOT_AVAILABLE',stage:'capabilities'});
   if(G.config.localModel.requireF16&&(cap.adapter?.f16!==true||cap.worker?.f16!==true))throw Object.assign(Error('El modo seguro requiere shader-f16 en la página y en el Worker.'),{code:'F16_REQUIRED_SAFE_MODE',stage:'capabilities'});
   const model=G.config.localModel.requireF16?cfg.model:(cap.adapter?.f16&&cap.worker?.f16!==false?cfg.model:cfg.f32);
   ...
   const result=await this.request('init',{model,runtimeURL:G.config.localModel.runtimeURL,runtimeBundled:G.config.localModel.runtimeBundled,contextWindowSize:G.config.localModel.contextWindowSize||1536,hardMaxOutputTokens:G.config.localModel.hardMaxOutputTokens||192,maxInputChars:G.config.localModel.maxInputChars||4600},G.config.localModel.loadTimeoutMs);
   if(ctl.signal.aborted)throw G.util.abortError();if(result?.liveInference!==true||result?.providerKind!=='local-llm'||result?.smoke?.responseNonEmpty!==true||result?.smoke?.schemaValid!==true||result?.smoke?.encodingValid!==true||result.worker!==true||result.adapterObtained!==true||result.modelId!==model)throw Object.assign(Error('El Worker no devolvió prueba válida de inferencia real.'),{code:'SMOKE_VALIDATION_FAILED',stage:'warming'});
   this.ready=true;this.liveInference=true;this.providerKind='local-llm';this.stage='ready';this.consecutiveFailures=0;this.lastLoad={...result,runtimeChecks:this.runtimeChecks.slice(),id:G.util.id('load'),pageId:G.RuntimeDiagnostics.pageId,at:new Date().toISOString(),profile,geniusPreparationTimeMs:performance.now()-start,capabilities:cap};void G.RuntimeDiagnostics.save(this.lastLoad);this.emit('ready',{message:'Genius listo. La generación interna fue comprobada.'});return this.lastLoad;
 ...
 async complete(messages,{schema,signal,temperature,maxTokens}={}){
  ...
  const wait=Math.max(0,(this.lastGenerationEndedAt+(G.config.localModel.generationCooldownMs||0))-performance.now());if(wait)await G.util.sleep(wait);if(signal?.aborted)throw G.util.abortError();
  ...
  try{const result=await this.request('complete',{messages,schema,temperature,maxTokens},G.config.localModel.generationTimeoutMs);if(signal?.aborted)throw G.util.abortError();this.consecutiveFailures=0;this.lastMetrics={firstTokenMs:result.firstTokenMs,totalGenerationMs:result.totalGenerationMs,tokensPerSecond:result.tokensPerSecond,tokensPerSecondScope:result.tokensPerSecondScope,usage:result.usage,worker:result.worker,model:result.model,finishReason:result.finishReason};return result}
  catch(e){if(e.name!=='AbortError'){this.consecutiveFailures++;this.diagnostic=G.RuntimeDiagnostics.classify(e,e.stage||'generation');const severe=/GPU|DEVICE|WORKER|STORAGE|TIMEOUT|MODEL_NOT_LOADED/.test(this.diagnostic.code)||this.consecutiveFailures>=G.config.localModel.maxConsecutiveFailures;if(severe)this.fail(e);else this.emit('generation-error',{message:this.diagnostic.cause,diagnostic:this.diagnostic})}throw e}
  finally{this.lastGenerationEndedAt=performance.now();signal?.removeEventListener('abort',abort);if(this.ready){this.stage='ready';this.emit('ready')}}
 }
 cancel(){
  ...
  const jobs=[...this.pending.values()].filter(p=>p.type==='complete');if(!jobs.length||this.status==='cancelling')return;for(const p of jobs)p.cancelled=true;this.emit('cancelling',{message:'Deteniendo la generación…'});this.worker?.postMessage({id:++this.sequence,type:'interrupt'});this.cancelTimer=setTimeout(()=>{this.dispose(G.util.abortError(),false);this.emit('cancelled',{message:'El Worker no confirmó la detención a tiempo y fue terminado. Prepara Genius de nuevo.'})},G.config.localModel.cancelTimeoutMs);
 }
