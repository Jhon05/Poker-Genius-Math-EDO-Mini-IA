...
const VERSION='7.2.1-rc1',RUNTIME_VERSION='0.2.85';
...
async function generate(m){const start=clock();let first=null,content='',usage=null,finishReason=null;const req={messages:m.messages,model:loadedModel,temperature:m.temperature??.35,top_p:.9,max_tokens:m.maxTokens||850,stream:true,stream_options:{include_usage:true},extra_body:{enable_thinking:false}};
 ...
 try{
  ...
  if(m.type==='init'){
   ...
   try{
    ...
    let parsed;try{parsed=JSON.parse(smoke.content)}catch(_){throw Error('Smoke test: respuesta JSON inválida.')}
