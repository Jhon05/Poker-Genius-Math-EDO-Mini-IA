...
async def main():
 async with async_playwright() as pw:
  ...
  check('all_profiles_selectable',await p.locator('#geniusCapProfile option').count()==3)
  check('not_blind_standard',await p.locator('#geniusCapProfile').input_value()=='light')
  ...
  await p.locator('#geniusCapProfile').select_option('advanced');check('manual_model_switch','4B' in await p.locator('#geniusModelName').inner_text())
  await p.locator('#geniusCapProfile').select_option('light');check('profile_synced_with_chat',await p.locator('#genieProfile').input_value()=='light')
 ...
 result={'version':'7.2.1-rc1','kind':'BROWSER_DOM_WITH_EXPLICIT_STORAGE_AND_LMS_FIXTURES','executed':True,'liveInferenceExecuted':False,'realBrightspaceExecuted':False,'total':len(rows),'passed':sum(r['pass_'] for r in rows),'failed':[r for r in rows if not r['pass_']],'results':rows}
