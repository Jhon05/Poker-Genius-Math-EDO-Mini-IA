...
out={'version':'7.2.1-rc1','tool':'sympy '+S.__version__,'kind':'EXACT_SYMBOLIC_CERTIFICATES_NOT_LLM','executed':True,'liveInferenceExecuted':False,'total':len(rows),'passed':sum(x['passed'] for x in rows),'failed':[x for x in rows if not x['passed']],'results':rows,'limitations':'These supplementary certificates do not independently certify all 500 questions, graphics or all alternative orderings. No bank content changed.'}
